...
class LLAMAmodel(torch.nn.Module):
    def __init__(self, config):
        ...
        self.embedding_dim = getattr(config.model, 'embedding_dim', 768)  # Default LLaMA hidden size
    ...
    def llama_model_string(self, model_size, chat):
        ...
        print_debug(my_print, "LLAMAmodel:llama_new_model_string:", model_string, True)
        return model_string
    ...
    def convert_time_series_to_embeddings(self, input_str, input_trend_str, input_season_str, input_resid_str, description=""):
        """
        ...
        """
        print_debug(my_print, "LLAMAmodel:convert_time_series_to_embeddings:input_str", input_str[:100], self.debug_mode)
        ...
        min_length = min(len(main_values), len(trend_values), len(season_values), len(resid_values))
        ...
        embeddings = projection_layer(time_series_matrix)  # Shape: [seq_len, embedding_dim]
        ...
        print_debug(my_print, "LLAMAmodel:convert_time_series_to_embeddings:output_shape", embeddings.shape, self.debug_mode)
        return embeddings
    ...
    def _parse_time_series_string(self, ts_string):
        ...
        try:
            ...
            print_debug(my_print, "LLAMAmodel:_parse_time_series_string:error", ts_string, self.debug_mode)
    ...
    def forecast_with_embeddings(self, model, tokenizer, input_str, input_trend_str, input_season_str, input_resid_str, steps, settings, batch_size, num_samples, temp, top_p):
        ...
        gen_strs = []
        print_debug(my_print, "LLAMAmodel:_forecast_with_embeddings:num_samples:", num_samples, self.debug_mode)
        print_debug(my_print, "LLAMAmodel:_forecast_with_embeddings:batch_size:", batch_size, self.debug_mode)
        ...
        for _ in tqdm(range(num_samples // batch_size)):
            ...
            with torch.no_grad():
                ...
                predictions = self._sample_from_embeddings(logits, steps, temp, top_p, settings, tokenizer)

            gen_strs.extend(predictions)
            print_debug(my_print, "LLAMAmodel:_forecast_with_embeddings:batch_predictions", len(predictions), self.debug_mode)

        print_debug(my_print, "LLAMAmodel:_forecast_with_embeddings:total_gen_strs", len(gen_strs), self.debug_mode)
        return gen_strs

    def _sample_from_embeddings(self, logits, steps, temp, top_p, settings, tokenizer):
        ...
        for i in range(batch_size):
            ...
            pred_str = self._convert_token_to_timeseries_value(token_id, tokenizer, steps, settings)
            predictions.append(pred_str)

        return predictions

    def _convert_token_to_timeseries_value(self, token_id, tokenizer, steps, settings):
    ...
    def forecast_with_tokens(self, model_name,
        ...
        gen_strs = []
        print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:num_samples:", num_samples, self.debug_mode)
        print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:batch_size:", batch_size, self.debug_mode)

        for _ in tqdm(range(num_samples // batch_size)):
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:_", _, self.debug_mode)
            batch = tokenizer(
                [input_str],
                return_tensors="pt",
            )

            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:batch first", batch, self.debug_mode)
            batch = {k: v.repeat(batch_size, 1) for k, v in batch.items()}
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:batch dict", batch, self.debug_mode)
            batch = {k: v.cuda() for k, v in batch.items()}
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:batch third", batch, self.debug_mode)
            num_input_ids = batch['input_ids'].shape[1]
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:num_input_ids", num_input_ids, self.debug_mode)
            good_tokens_str = list("0123456789" + settings.time_sep)
            good_tokens = [tokenizer.convert_tokens_to_ids(token) for token in good_tokens_str]
            bad_tokens = [i for i in range(len(tokenizer)) if i not in good_tokens]

            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:batch input", batch, self.debug_mode)
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:max_tokens", max_tokens, self.debug_mode)
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:temp", temp, self.debug_mode)
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:top_p", top_p, self.debug_mode)
            array_bad_tokens = [[t] for t in bad_tokens]
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:bad_words_ids", array_bad_tokens[0:3], self.debug_mode)
            ...
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:generate_ids", generate_ids, self.debug_mode)
            gen_strs += tokenizer.batch_decode(
                generate_ids[:, num_input_ids:],
                skip_special_tokens=True,
                clean_up_tokenization_spaces=False
            )
            print_debug(my_print, "LLAMAmodel:_forecast_with_tokens:gen_strs", gen_strs, self.debug_mode)
        return gen_strs
