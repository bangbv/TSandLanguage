...
class LLAMAmodel(torch.nn.Module):
  ...
  def convert_time_series_to_embeddings(self,
    ...
    print_debug(my_print,
                "LLAMAmodel:convert_time_series_to_embeddings:input_batch_length", len(input_batch), self.debug_mode)
    print_debug(my_print,
  ...
  def forecast_with_embeddings(self, model_name,
    ...
    for _ in tqdm(range(num_samples // batch_size)):
      ...
      with torch.no_grad():
        ...
        # gen_strs_decode = tokenizer.batch_decode(
        #     generation_id_trunk,
        #     skip_special_tokens=True,
        #     clean_up_tokenization_spaces=False
        # )
        # print_debug(my_print,
        #             "LLAMAmodel:_forecast_with_embeddings:gen_strs_decode:",
        #             gen_strs_decode, self.debug_mode)

      gen_strs.extend(generation_id_trunk)
      print_debug(my_print,
                  "LLAMAmodel:_forecast_with_embeddings:gen_strs",
  ...
  def _sample_from_embeddings(self, logits, steps, temp, top_p, settings,
    ...
    print_debug(my_print, "LLAMAmodel:_sample_from_embeddings:batch_size:", batch_size,
                self.debug_mode)
    pred_strs = []

    # Get the last token predictions for each sequence in batch
    last_logits = logits[:, -1, :]  # [batch_size, vocab_size]
    print_debug(my_print, "LLAMAmodel:_sample_from_embeddings:last_logits:", last_logits,
                self.debug_mode)

    # Apply temperature scaling
    last_logits = last_logits / temp
    print_debug(my_print, "LLAMAmodel:_sample_from_embeddings:last_logits_temperature_scaling:", last_logits,
                self.debug_mode)
    # Apply top-p sampling
    sorted_logits, sorted_indices = torch.sort(last_logits, descending=True)
    print_debug(my_print, "LLAMAmodel:_sample_from_embeddings:sorted_logits:", sorted_logits,
                self.debug_mode)
    print_debug(my_print, "LLAMAmodel:_sample_from_embeddings:sorted_indices:", sorted_indices,
                self.debug_mode)
    cumulative_probs = torch.cumsum(F.softmax(sorted_logits, dim=-1), dim=-1)
    print_debug(my_print, "LLAMAmodel:_sample_from_embeddings:cumulative_probs:", cumulative_probs,
                self.debug_mode)
