...
def deserialize_str(bit_str, settings: dict, ignore_last=False, steps=None):
    """
...
def ori_scale_deserialize(completion):
    string_list = completion.split(",")
    pred_arr = []
    for item in string_list:
        try:
            pred_arr.append(float(item))
        except:
            continue
    return pred_arr
    
def ori_scale_serialize(arrs ):
    '''
        To prevent the length of the input string from becoming too long due to 
        excessive decimal places, we struck a balance between removing decimals and 
        retaining precision.
    '''
    input_strs = []
    for arr in arrs:
        ...
        for a in arr : 
            if abs(a) >= 1.0 : 
                ...
                a = round(a,2)
            if inp_str == '': inp_str = str(a)
            else : inp_str += ', ' + str(a)
        input_strs.append(inp_str)
    input_strs = tuple(input_strs)
    return input_strs 